window.onload = function () {
    ...
    $('.order_form select').change(function () {
        ...
        if (orderitem_product_pk) {
            $.ajax({
                url: '/orders/getprice/' + orderitem_product_pk + '/',
                success: function (data) {
                    if (data.price) {
                        ...
                        let row = $('.order_form table').find('tr:eq(' + (orderitem_num + 1) + ')');
                        row.find('td:eq(2)').html(price_html);
                        console.log(row.find('td:eq(1)').val());
                        orderSummaryRecalc();
                    }
                }
            });
